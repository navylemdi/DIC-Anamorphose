...
if S.SurfaceType=='Plan':
    ...
    yf, zf  = np.meshgrid(np.arange(min(CadreAileUnfolded[:,0]), max(CadreAileUnfolded[:,0]) + widthPrintable, widthPrintable), np.arange(min(CadreAileUnfolded[:,1]), max(CadreAileUnfolded[:,1]) + heightPrintable, heightPrintable))
...
if S.SurfaceType == 'Plan':
    ...
    for i in range(debut, len(Feuille1.contours), saut):
          plt.plot(UnfoldedPnt1[i][:, 0], UnfoldedPnt1[i][:, 1], color='black')
          plt.fill(UnfoldedPnt1[i][:, 0], UnfoldedPnt1[i][:, 1], color='black')
    # for i in range(debut2, len(Feuille2.contours), saut):
    #       plt.plot(UnfoldedPnt2[i][:, 1], UnfoldedPnt2[i][:, 2], color='blue')
    #       plt.fill(UnfoldedPnt2[i][:, 1], UnfoldedPnt2[i][:, 2], color='blue')
    # for i in range(debut3, len(Feuille3.contours), saut):
    ...
    #       plt.plot(UnfoldedPnt5[i][:, 1], UnfoldedPnt5[i][:, 2], color='g')
    #       plt.fill(UnfoldedPnt5[i][:, 1], UnfoldedPnt5[i][:, 2], color='g')  
    plt.scatter(CadreAileUnfolded[:,0], CadreAileUnfolded[:,1], color='c', marker='+')
    plt.scatter( yf, zf, marker='+', color='m')
    for i in range (yf.shape[0]-1):
        for j in range (yf.shape[1]-1):
            plt.text((yf[i,j]+yf[i,j+1])/2, (zf[i,j]+zf[i+1,j])/2, str((i+1)*(j+1)), color='black')
    plt.title('Dépliée')
    #plt.axis('equal')
    plt.xlim(min(CadreAileUnfolded[:,0]), max(CadreAileUnfolded[:,0]))
    plt.ylim(min(CadreAileUnfolded[:,1]), max(CadreAileUnfolded[:,1]))
    plt.grid()
    plt.show()
