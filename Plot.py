...
class Plot:
    ...
    def Plot3D(Nbimage, debut, saut, Liste_Feuille, Liste_Projection, CadreAile, d):
        fig = plt.figure(Nbimage+1)
        ax = fig.add_subplot(111, projection='3d')
        ax.scatter(0, 0, 0, color='b')
        for j in range(Nbimage):
            for i in range (debut, len(Liste_Feuille[j].contours), saut):
                ...
                ax.scatter([d]*4, Liste_Feuille[j].Cadre[:,0], Liste_Feuille[j].Cadre[:,1], color='k', marker='+')
    ...
    def PlotReference(Nbimage, debut, saut, Liste_Feuille):
        for i in range(Nbimage):
            fig=plt.figure(i+1)
            ax = fig.add_subplot(111, aspect='equal')
            for j in range(debut, len(Liste_Feuille[i].contours), saut):
    ...
    def PlotUnfolded(Nbimage, debut, saut, Liste_Feuille, Liste_depliage, CadreAileUnfolded, yf, zf):
